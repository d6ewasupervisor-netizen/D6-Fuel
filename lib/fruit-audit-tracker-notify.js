...
async function sendPledgeSignedUp(resend, { pledge, meta, deadline, dashboardUrl, recipients }) {
  const subject = `D1 fruit audit FM ${pledge.storeId} assigned - ${pledge.name}`;
  const html = emailShell(
    'New District 1 fruit audit assignment',
    `
      <p style="margin:0 0 16px"><strong>${escapeHtml(pledge.name)}</strong> was assigned this District 1 fruit audit store.</p>
...
async function sendPledgeReleased(resend, { pledge, meta, dashboardUrl, recipients }) {
  const subject = `D1 fruit audit FM ${pledge.storeId} assignment released - ${pledge.name}`;
  const html = emailShell(
    'District 1 fruit audit assignment released',
    `
      <p style="margin:0 0 16px"><strong>${escapeHtml(pledge.name)}</strong> released their fruit audit assignment.</p>
      <table style="border-collapse:collapse;font-size:14px;width:100%">
        <tr><td style="padding:6px 12px 6px 0;color:#666">Store</td><td><strong>${escapeHtml(storeLine(meta))}</strong></td></tr>
        <tr><td style="padding:6px 12px 6px 0;color:#666">Auditor</td><td>${escapeHtml(pledge.name)}</td></tr>
...
async function sendOpeningsAvailable(resend, { releasedPledges, metas, dashboardUrl, recipients, cc }) {
  ...
  const html = emailShell(
    ...
    `
      <p style="margin:0 0 16px">${pledges.length === 1 ? 'An assigned District 1 fruit audit store is' : 'Assigned District 1 fruit audit stores are'} open again. If you can help, take ${pledges.length === 1 ? 'it' : 'one'} on the live dashboard:</p>
...
async function sendCompletionReceived(resend, { completion, meta, completedCount, completedStores, dashboardUrl, cc }) {
  ...
  const stores = Array.isArray(completedStores) && completedStores.length
    ? completedStores
    : [completion.storeId];
  const subject = `D1 fruit audit FM ${completion.storeId} complete`;
  const html = emailShell(
