...
async function sendEmail(resend, { to, cc, subject, html, from, replyTo }) {
  ...
  const payload = {
    from: from || process.env.FRUIT_AUDIT_TRACKER_NOTIFY_FROM || 'D1 Fruit Audit Tracker <fruitaudit@the-dump-bin.com>',
    to: toList,
    subject,
    html,
  };
...
async function sendAssigneeAssignmentNotice(resend, {
  ...
  return sendEmail(resend, { to: [pledge.email], cc, subject, html });
}
...
function trackerStoreStatusRows(stores) {
  const openRows = [];
  const completeRows = [];
  (stores || []).forEach(store => {
    const label = `FM ${escapeHtml(store.id)}`;
    if (store.status === 'complete') completeRows.push(`<li style="margin:4px 0">${label}</li>`);
    else if (store.status === 'open') openRows.push(`<li style="margin:4px 0"><strong>${label}</strong> — available now</li>`);
  });
  return { openRows: openRows.join(''), completeRows: completeRows.join('') };
}

async function sendD1FruitAuditVolunteerAppeal(resend, {
  storeId,
  recipients,
  preferredStoreId,
  preferredStoreLabel,
  ...
  const store = String(storeId || '').replace(/\D/g, '').padStart(3, '0');
  const preferred = String(preferredStoreId || store).replace(/\D/g, '').padStart(3, '0');
  const to = uniqueEmails(recipients);
  if (!to.length) return { ok: false, skipped: true, reason: 'no_recipients' };

  const dashboard = dashboardUrl || 'https://fuel.retail-odyssey.com/fruit-audit-dashboard?district=1';
  const fieldUrl = fieldAppUrlForStore(fieldAppUrl, preferred, '1');
  const { openRows, completeRows } = trackerStoreStatusRows(snapshot && snapshot.stores);
  const tysonEmail = fromEmail || 'tyson.gauthier@retailodyssey.com';
  const sendFrom = process.env.FRUIT_AUDIT_TYSON_SEND_FROM || 'Updates@retail-odyssey.com';
  const subject = `District 1 fruit audit — help needed at FM ${store} (and other open stores)`;
  const html = emailShell(
    'District 1 fruit audit — your help is needed',
    `
      <p style="margin:0 0 16px">Hi team,</p>
      <p style="margin:0 0 16px">I'm reaching out because we still need fruit table audit photos for several District 1 stores. If you are at Fred Meyer today, would you please help us complete this project?</p>
      <p style="margin:0 0 16px">If you are scheduled at <strong>FM ${escapeHtml(preferred)}</strong>${preferredStoreLabel ? ` (${escapeHtml(preferredStoreLabel)})` : ''}, please start there. Other stores are also open if you have capacity to pick up an additional location.</p>
      ${d1PhotoRoutingNoticeHtml()}
      <p style="margin:0 0 8px;font-size:13px;color:#444"><strong>Live tracker — open stores</strong></p>
      <ul style="padding-left:20px;margin:0 0 16px;font-size:14px;line-height:1.55">${openRows || '<li style="margin:4px 0">See the dashboard for current availability.</li>'}</ul>
      ${completeRows ? `<p style="margin:0 0 8px;font-size:13px;color:#444"><strong>Already complete</strong></p><ul style="padding-left:20px;margin:0 0 16px;font-size:14px;line-height:1.55;color:#666">${completeRows}</ul>` : ''}
      <p style="margin:0 0 8px;font-size:13px;color:#444"><strong>How to help</strong></p>
      <ol style="padding-left:20px;margin:0 0 16px;font-size:14px;line-height:1.65;color:#333">
        <li>Open the <a href="${escapeHtml(dashboard)}">District 1 assignment dashboard</a> to see which stores are open and which are done.</li>
        <li>Assign yourself to the store you will cover (FM ${escapeHtml(preferred)} first if you are working there today).</li>
        <li>Open the <a href="${escapeHtml(fieldUrl)}">fruit photo app</a>, capture all required 360-degree fruit set photos, and submit.</li>
      </ol>
      <div style="margin:16px 0 0;padding:12px 14px;border-left:4px solid #2563eb;background:#f0f6ff;border-radius:6px;color:#1a2f55;font-size:13px;line-height:1.6">
        <p style="margin:0">I will make sure you are compensated fairly for your time on this audit. If you are not on shift or have questions about payment, reply to this email and I will take care of it.</p>
      </div>
      <p style="margin:16px 0 0;font-size:13px;color:#444">Thank you for helping us wrap this up.</p>
      <p style="margin:12px 0 0;font-size:13px;color:#444">— Tyson Gauthier<br>District 1 Supervisor</p>
    `,
    dashboard,
  );
  ...
  return sendEmail(resend, {
    to,
    cc: uniqueEmails([...(cc || []), tysonEmail]),
    subject,
    html,
    from: `Tyson Gauthier <${sendFrom}>`,
    replyTo: tysonEmail,
  });
}
...
module.exports = {
  ...
  sendD1FruitAuditVolunteerAppeal,
  assignmentReviewUrl,
  notifyRecipients,
  d1MonitorCcEmails,
  d1PhotoRoutingNoticeHtml,
