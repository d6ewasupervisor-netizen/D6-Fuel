...
async function sendCompletionHoursEarned(resend, { completion, meta, totalHours, completedStores, dashboardUrl, cc }) {
  ...
  const stores = Array.isArray(completedStores) && completedStores.length
    ? completedStores
    : [completion.storeId];
  const subject = `D1 fruit audit FM ${completion.storeId} complete - ${hours} hour${hours === 1 ? '' : 's'} earned`;
  const html = emailShell(
    ...
    `
      ...
      <table style="border-collapse:collapse;font-size:14px;width:100%">
        ...
        <tr><td style="padding:6px 12px 6px 0;color:#666">This submission</td><td><strong>1 hour earned</strong></td></tr>
        <tr><td style="padding:6px 12px 6px 0;color:#666">Total District 1 fruit audit hours</td><td><strong>${hours}</strong></td></tr>
