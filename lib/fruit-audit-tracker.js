...
const DEFAULT_DEADLINE_ISO = '2026-06-13T17:00:00-07:00';
const PREVIOUS_DEFAULT_DEADLINE_ISOS = new Set([
  '2026-06-12T19:00:00-07:00',
  '2026-06-19T23:59:59-07:00',
]);
const TRACKER_TIME_ZONE = 'America/Los_Angeles';
...
function completionWeekday(completedAt) {
...
function earnedHoursForCompletion(completion) {
  const weekday = completionWeekday(completion && completion.completedAt);
  if (weekday === 'Sat') return 0.5;
  return 1;
}
...
function buildSnapshot() {
  ...
  stores.forEach(store => {
    ...
    const storeHours = earnedHoursForCompletion(store.completion);
    store.completion.earnedHours = storeHours;
    earnedHours += storeHours;
    ...
    hoursByEmail[key].hours += storeHours;
