...
class DistrictFruitAuditTracker {
    ...
    try {
      if (fs.existsSync(this.dataPath)) {
        ...
        return {
          ...
          manualOpenStores: Array.isArray(raw.manualOpenStores) ? raw.manualOpenStores : [],
        };
      }
      ...
      if (seedPath && fs.existsSync(seedPath)) {
        ...
        return {
          ...
          manualOpenStores: Array.isArray(seed.manualOpenStores) ? seed.manualOpenStores : [],
        };
      }
    } catch (err) {
  ...
  pruneRemovedStoreState({ persistChanges = false } = {}) {
    ...
    for (const completionKey of Object.keys(this.state.completions || {})) {
      const storeId = padAnyStoreId(String(completionKey || '').split('::')[0]);
      if (!storeId || !this.storeIds.has(storeId)) {
        delete this.state.completions[completionKey];
        changed = true;
      }
    }
    if (changed) {
  ...
  syncDefaultAssignments({ persistChanges = false } = {}) {
    ...
    const manualOpen = new Set((this.state.manualOpenStores || []).map(padAnyStoreId).filter(Boolean));
    for (const assignment of this.defaultAssignments) {
      ...
      if (manualOpen.has(pledge.storeId)) continue;
  ...
  getCompletionForPledge(storeId, email) {
    ...
    if (!legacy.email && (legacy.source === 'destination-folder' || legacy.source === 'manual-seed')) {
      return legacy;
    }
    return null;
  }
  ...
  /**
   * Release incomplete assignments and keep stores open for volunteer self-assign.
   * Preserves completion records and does not re-seed default assignments.
   */
  openStoresForVolunteerSignup(storeIds, { persistChanges = true } = {}) {
    if (!this.state) throw new Error('Tracker not initialized.');
    const manualOpen = new Set((this.state.manualOpenStores || []).map(padAnyStoreId).filter(Boolean));
    const released = [];
    const opened = [];
    const skipped = [];
    ...
    for (const rawStoreId of storeIds || []) {
      const store = this.padStoreId(rawStoreId);
      if (!store) {
        skipped.push({ storeId: rawStoreId, reason: 'invalid_store' });
        continue;
      }
      if (this.isStoreFullyComplete(store)) {
        skipped.push({ storeId: store, reason: 'already_complete' });
        continue;
      }
      released.push(...this.forceReleaseActivePledgesForStore(store));
      manualOpen.add(store);
      opened.push(store);
    }

    this.state.manualOpenStores = [...manualOpen].sort();
    const openedSet = new Set(opened);
    const resolvedAt = new Date().toISOString();
    for (const request of this.state.assignmentRequests || []) {
      if (request.status !== 'pending' || !openedSet.has(request.storeId)) continue;
      request.status = 'denied';
      request.resolvedAt = resolvedAt;
      request.resolvedBy = 'tyson.gauthier@retailodyssey.com';
      request.resolutionNote = 'Store opened for volunteer signup on the live dashboard.';
    }
